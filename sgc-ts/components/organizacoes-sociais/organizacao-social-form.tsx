...
interface OrganizacaoSocialFormProps {
  orgToEdit?: OrganizacaoSocialDto | null
  onSuccess: () => void
}

export function OrganizacaoSocialForm({ orgToEdit, onSuccess }: OrganizacaoSocialFormProps) {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: (orgToEdit as any) || {},
  })

  useEffect(() => {
    if (orgToEdit) {
      form.reset(orgToEdit as any)
    }
  }, [orgToEdit, form])
  ...
  const onSubmit = (values: FormValues) => {
    ...
    mutation.mutate(
        ...
        : values,
      {
        ...
        onError: (error: any) => {
          toast({ title: "Erro ao salvar organização", description: error.message, variant: "destructive" })
        },
      }
    )
  }
