...
export function AgendamentoPagamentos() {
  ...
  const despesasAprovadas: any[] = [] // Array vazio para simular ausência de dados

  const getVencimentosPorDia = () => {
    const vencimentos: { [key: string]: number } = {}
    // Lógica para preencher vencimentos com dados reais
    return vencimentos
  }

  const getDespesasDoDia = (data: Date) => {
    // Lógica para filtrar despesas reais por data
    return []
  }
  ...
  const handleAgendarLote = () => {
    toast({
      ...
      description: `MOCK ${selectedItems.length} pagamentos agendados. Total: R$ 0,00`,
    })
    setSelectedItems([])
  }
  ...
  const handleMarcarPago = (id: string) => {
    toast({
      ...
      description: `MOCK Pagamento confirmado.`,
    })
  }
                ...
                <div className="space-y-3">
                  {getDespesasDoDia(selectedDate).map((despesa) => (
                    <div key={despesa.id} className="p-3 border rounded-lg">
                      <div className="font-medium text-sm">MOCK</div>
                      <div className="text-xs text-gray-600">MOCK</div>
                ...
                <div key={despesa.id} className="flex items-center space-x-4 p-4 border rounded-lg hover:bg-gray-50">
                  ...
                  <div className="flex-1 grid grid-cols-1 md:grid-cols-5 gap-4 items-center">
                    <div>
                      ...
                      <div className="text-sm">MOCK</div>
                    </div>
                    ...
                    <div className="text-center">
                      <Button size="sm" onClick={() => handleMarcarPago("MOCK")}>
